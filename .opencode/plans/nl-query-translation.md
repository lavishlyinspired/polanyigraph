...
- [ ] A DSL-syntax query (e.g. `regulates("FINMA", X)`) sent to the agent runs exactly as today — same result, no translation call, no added latency
- [ ] A natural-language question (e.g. "who does FINMA regulate?") is translated to a real DSL query grounded in this graph's live ontology schema, actual predicate/entity vocabulary, graph-native few-shot examples, and recent conversation history, then executed and answered
- [ ] An out-of-scope question returns the `NL_QUERY_OUT_OF_SCOPE` sentinel, not a fabricated best-effort query
- [ ] A follow-up question resolves against the prior turn via threaded conversation history
- [ ] The generated DSL query is visible to the user in the agent's reply
- [ ] A failed generation gets exactly one retry with the parser error fed back before falling back to the syntax-explanation message
- [ ] A confirmed-good translation can be promoted into that repository's `:FewshotQuery` set
- [ ] No changes to `query_engine.py`, `api/query.py`, or `QueryPage.tsx` (asserted as literal no-diff checks)
- [ ] `kg-query/SKILL.md` reflects the new primary behavior (state the query that ran) with syntax-explanation demoted to fallback
- [ ] All code has tests (unit for `translate_to_dsl` against a fixture `OntologySchema` + fake `LLMClient`, integration for the full `querier_node` path including the retry branch and the feedback-promotion path)
...
### Slice 1: `translate_to_dsl` + graph-native few-shot store (walking skeleton)
...
### Slice 2: Wire into `querier_node` — skip-if-already-DSL, conversation history
...
### Slice 3: Retry-once-on-error + fallback
...
### Slice 4: Feedback capture + few-shot promotion
...
### Slice 5: Transparency in the reply + skill update
