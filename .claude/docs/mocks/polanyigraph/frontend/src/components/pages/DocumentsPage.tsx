...
import { 
  Zap, Info, FileText, Check, X, Loader, ChevronDown, ChevronRight, 
  Trash2, AlertCircle, Clock, BookOpen, Search, Sparkles
} from 'lucide-react';
...
export function DocumentsPage() {
  const { 
    loading, error, ingest, history,
    pendingFacts, approvedFacts, enriching, enrich, approveFact, rejectFact, loadGraph
  } = useGraphStore();
  ...
  const filteredHistory = history.filter(event => 
    event.text.toLowerCase().includes(docFilter.toLowerCase())
  );
  ...
  return (
    <div className="h-full flex-1 flex overflow-hidden bg-zinc-950">
      {/* 1. LEFT COLUMN - Ingestion History & Stats */}
      <div className="w-80 border-r border-zinc-800 flex flex-col min-w-0 bg-zinc-950/20">
        <div className="p-4 border-b border-zinc-800 bg-zinc-900/10 flex shrink-0 justify-between items-center">
          ...
          <div className="text-[10px] font-mono text-indigo-400 font-semibold px-2 py-0.5 rounded bg-indigo-500/10 border border-indigo-500/20">
        ...
        <div className="p-3 border-b border-zinc-800 shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded pl-8 pr-2.5 py-1.5 text-[11px] text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-indigo-500 transition-colors"
            />
          </div>
        </div>
            ...
            filteredHistory.map((event) => (
              <div
                ...
                className={`p-3 rounded-lg border cursor-pointer transition-all ${
                  selectedDoc?.id === event.id
                    ? 'border-indigo-500 bg-indigo-500/5'
                    : 'border-zinc-800 bg-zinc-900/40 hover:bg-zinc-900/80 hover:border-zinc-700'
                }`}
              >
                <div className="text-[11px] text-zinc-200 line-clamp-3 font-sans leading-relaxed">
                  {event.text}
                </div>
      ...
      {/* 2. MIDDLE COLUMN - Ingestion & Extraction Workstation */}
      <div className="flex-1 border-r border-zinc-800 flex flex-col min-w-0">
        ...
        {/* Selected Doc Viewer Detail */}
        {selectedDoc ? (
          <div className="m-4 p-4 rounded-lg border border-indigo-500/30 bg-indigo-500/5 space-y-3 shrink-0">
            ...
            <div className="text-xs text-zinc-300 leading-relaxed max-h-36 overflow-y-auto font-sans p-2 rounded bg-zinc-950/40 border border-zinc-800">
              {selectedDoc.text}
            </div>
            <div className="flex items-center gap-4 text-[10px] font-mono text-zinc-500">
              <span>Heuristic Anchor: <strong className="text-zinc-300">{selectedDoc.entityCount} nodes</strong></span>
              <span>Relationships: <strong className="text-zinc-300">{selectedDoc.relationshipCount} edges</strong></span>
              {selectedDoc.droppedCount > 0 && (
                <span className="text-amber-400">Heuristics Dropped: <strong>{selectedDoc.droppedCount}</strong></span>
              )}
            </div>
          </div>
        ) : (
          <div className="m-4 p-3 rounded-lg border border-zinc-800 bg-zinc-900/40 flex items-start gap-2.5 shrink-0">
            ...
            <div className="text-[10px] text-zinc-400 leading-relaxed">
              <strong className="text-zinc-200">Polanyi Ingestion Pipeline:</strong> Select a loaded document from the left list to view its registered extraction trace, or paste a new source segment below to trigger structured ontological processing.
            </div>
          </div>
        )}
        ...
        <div className="flex-1 flex flex-col p-4 gap-4 min-h-0">
          <div className="flex flex-col gap-2">
            ...
            <div className="relative flex-1 min-h-[250px]">
              <textarea
                ...
                className="w-full h-full min-h-[240px] bg-zinc-950 border border-zinc-800 rounded-lg p-3.5 text-xs text-white placeholder:text-zinc-700 resize-none focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all font-sans leading-relaxed"
              />
              {charCount > 0 && (
          ...
          <div className="flex gap-3 shrink-0">
            <button
              ...
              className="flex-1 h-10 bg-white text-black hover:bg-zinc-200 text-xs font-bold rounded-lg flex items-center justify-center gap-2 disabled:opacity-40 transition-all"
            >
              {loading ? (
                <>
                ...
                <>
                  <Zap className="w-4 h-4 fill-black" /> Run Document Knowledge Extraction
                </>
              )}
            </button>
            {text.trim() && (
              <button
                onClick={handleClear}
                className="h-10 px-4 rounded-lg border border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-900 transition-colors"
                title="Clear content"
              >
      ...
      {/* 3. RIGHT COLUMN - Polanyi Cognitive Enrichment Queue */}
      <div className="w-96 border-l border-zinc-800 flex flex-col min-w-0 bg-zinc-950/40">
        <div className="p-4 border-b border-zinc-800 bg-zinc-900/10 flex shrink-0 justify-between items-center">
          <div>
            <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
              <Sparkles className="w-3.5 h-3.5 text-violet-400" /> Implicit Facts review
            </h3>
            <p className="text-[9px] text-zinc-500 mt-0.5">Polanyi Tacit Knowledge Heuristics</p>
          </div>
          <span className="px-2 py-0.5 rounded text-[9px] bg-violet-500/10 border border-violet-500/30 text-violet-400 font-mono font-bold">
        ...
        <div className="p-3 border-b border-zinc-800 shrink-0 bg-zinc-950/20">
          ...
          >
            {enriching ? (
              ...
              <>
                <Sparkles className="w-3 h-3 text-violet-400" /> Trigger Heuristic Enrichment
              </>
            )}
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0">
          <div className="space-y-2">
            <div className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider font-mono">
              Pending Enrichment queue ({sortedPending.length})
            </div>
                ...
                {sortedPending.map((fact) => {
                  ...
                  return (
                    <div 
                      ...
                      style={{ borderColor: `hsl(${hue}, 45%, 25%)`, backgroundColor: `hsl(${hue}, 45%, 6%)` }}
                    >
                      <div className="flex items-center justify-between">
                        <span 
                          className="px-2 py-0.5 rounded text-[8px] font-mono font-bold uppercase tracking-wider" 
                          style={{ backgroundColor: `hsl(${hue}, 55%, 45%)`, color: '#000' }}
                        >
                      ...
                      <div className="flex gap-2 border-t border-zinc-800/40 pt-2 mt-1">
                        <button 
                          ...
                          className="flex-1 h-6.5 rounded bg-emerald-500/10 border border-emerald-500/25 text-emerald-400 text-[10px] font-bold flex items-center justify-center gap-1 hover:bg-emerald-500/25 transition-colors"
                        >
                          <Check className="w-3 h-3" /> Approve
                        </button>
                        <button 
                          onClick={() => void rejectFact(fact.id)} 
                          className="flex-1 h-6.5 rounded bg-rose-500/10 border border-rose-500/25 text-rose-400 text-[10px] font-bold flex items-center justify-center gap-1 hover:bg-rose-500/25 transition-colors"
                        >
                ...
                <div className="space-y-2 mt-3">
                  {approvedFacts.map((fact) => {
                    ...
                    return (
                      <div 
                        key={fact.id} 
                        className="p-3 rounded-lg border space-y-1.5 bg-zinc-900/20 border-zinc-800/80"
                      >
                        <div className="flex items-center justify-between">
