...
import { 
  Brain, Zap, Sparkles, RefreshCw, Trash2, Play, Square, 
  Layers, GitFork, ChevronRight, CheckCircle2, AlertCircle, Clock, 
  TrendingUp, Terminal, TerminalSquare, Info, ShieldAlert
} from 'lucide-react';
...
export function InferencePage() {
  ...
  return (
    <div className="h-full flex-1 flex overflow-hidden bg-zinc-950">
      ...
      {/* 1. LEFT COLUMN - Neurosymbolic Control Deck */}
      <div className="w-[360px] border-r border-zinc-800 flex flex-col min-w-0 bg-zinc-950/20">
        <div className="p-4 border-b border-zinc-800 bg-zinc-900/10 shrink-0">
          <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
            <Brain className="w-4 h-4 text-indigo-400" /> NS-Loop Control Deck
          </h3>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0">
          ...
          <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-950/40 space-y-3">
            ...
            <div className="flex gap-2 pt-1.5">
              {autoRunning ? (
                <button
                  onClick={stopAutoRun}
                  className="flex-1 h-8 rounded bg-rose-600 text-white hover:bg-rose-700 text-[11px] font-bold flex items-center justify-center gap-1.5 transition-colors"
                >
                ...
                <button
                  ...
                  className="flex-1 h-8 rounded bg-white text-black hover:bg-zinc-200 text-[11px] font-bold flex items-center justify-center gap-1.5 disabled:opacity-30 transition-all"
                >
                  <Play className="w-3 h-3 fill-black" /> Auto-Run Loop
                </button>
              )}
              <button
              ...
              <button
                ...
                className={`h-8 px-2.5 rounded border text-xs transition-colors ${showProofPath ? 'bg-violet-400/15 border-violet-400/40 text-violet-400' : 'bg-zinc-900 border-zinc-800 text-zinc-500 hover:text-white'}`}
          ...
          <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-950/40 space-y-3">
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-lg text-xs text-white h-9 px-3 pr-8 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all appearance-none cursor-pointer"
                >
            ...
            <button
              ...
              className="w-full h-8.5 bg-zinc-800 hover:bg-zinc-750 text-zinc-200 border border-zinc-700 hover:text-white text-xs font-bold rounded-lg flex items-center justify-center gap-1.5 disabled:opacity-30 transition-all"
            >
              <Zap className="w-3.5 h-3.5 text-indigo-400" /> Spread Activation
            </button>
          </div>

          {/* Phase 2: Symbolic Inference */}
          <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-950/40 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider font-mono flex items-center gap-1">
            ...
            <button
              ...
              className="w-full h-8.5 bg-zinc-800 hover:bg-zinc-750 text-zinc-200 border border-zinc-700 hover:text-white text-xs font-bold rounded-lg flex items-center justify-center gap-1.5 disabled:opacity-30 transition-all"
            >
              <Sparkles className="w-3.5 h-3.5 text-emerald-400" /> Run Logical Inference
            </button>
            {!hasActivation && (
              <p className="text-[9px] text-zinc-600 flex items-center gap-1"><Info className="w-3 h-3" /> Requires active spread activation levels first.</p>
            )}
          </div>
          ...
          <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-950/40 space-y-3">
            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider font-mono flex items-center gap-1">
              <RefreshCw className="w-3.5 h-3.5 text-violet-400" /> 3. Feedback Loop
            </span>
            ...
            <button
              ...
              className="w-full h-8.5 bg-zinc-800 hover:bg-zinc-750 text-zinc-200 border border-zinc-700 hover:text-white text-xs font-bold rounded-lg flex items-center justify-center gap-1.5 disabled:opacity-30 transition-all"
            >
              <RefreshCw className="w-3.5 h-3.5 text-violet-400" /> Feed Back To Neural {pendingFeedback > 0 ? `(${pendingFeedback})` : ''}
            </button>
            {!hasFacts && (
              <p className="text-[9px] text-zinc-600 flex items-center gap-1"><Info className="w-3 h-3" /> Requires derived facts to feedback.</p>
            )}
          </div>

          {/* Convergence run shortcut */}
          <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-950/40 space-y-2.5">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider font-mono flex items-center gap-1">
                <TerminalSquare className="w-3.5 h-3.5 text-indigo-400" /> Run to Convergence
              </span>
              {convergedBy && (
            ...
            <p className="text-[9px] text-zinc-600 leading-relaxed">Runs Spread Activation, Symbolic Inference and Neural Feedback stages iteratively in single transaction until fixed-point stability is reached.</p>
            <button
              onClick={() => void reason()}
              disabled={!selectedNode || loading || autoRunning}
              className="w-full h-9 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-lg flex items-center justify-center gap-1.5 disabled:opacity-30 transition-colors"
            >
              {loading ? (
                <>
                ...
                <>
                  <Play className="w-3.5 h-3.5 fill-white" /> Compute Convergence Loop
                </>
              )}
            </button>
            {iterations > 0 && (
              <p className="text-[8px] text-indigo-400 font-mono text-right">Resolved in {iterations} rounds.</p>
            )}
          </div>
      ...
      {/* 2. MIDDLE COLUMN - Diagnostic Logs & Activation Levels */}
        ...
        <div className="p-4 border-b border-zinc-800 bg-zinc-900/10 shrink-0 flex items-center justify-between">
          <div>
            <h2 className="text-[13px] font-bold text-white tracking-wider uppercase flex items-center gap-2">
              <Terminal className="w-4 h-4 text-indigo-400" /> Cognitive Execution Tracer
            </h2>
        ...
        <div className="flex-1 p-4 flex flex-col gap-4 overflow-hidden min-h-0">
          <div className="flex-1 flex flex-col border border-zinc-800 rounded-xl bg-zinc-950 overflow-hidden">
            <div className="h-9 border-b border-zinc-800 bg-zinc-900/40 flex items-center justify-between px-3 shrink-0 font-mono text-[10px] text-zinc-500">
              <span className="flex items-center gap-1.5"><Terminal className="w-3.5 h-3.5 text-zinc-500" /> Diagnostic stdout trace</span>
            ...
            <div className="flex-1 overflow-y-auto p-4 font-mono text-[11px] text-zinc-300 space-y-2.5 min-h-0 bg-zinc-950/40">
              {trace.length === 0 ? (
                ...
                trace.map((t, i) => (
                  <div
                    key={i}
                    className={`p-3 rounded-lg border text-[11px] flex items-start gap-3 transition-colors ${
                      t.fired ? 'border-amber-500/20 bg-amber-500/5' : 'border-zinc-900 bg-zinc-900/20'
                    }`}
                  >
                    {t.fired ? (
                      <CheckCircle2 className="w-4 h-4 text-amber-400 shrink-0 mt-0.5 animate-pulse" />
                    ) : (
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="px-1.5 py-0.5 rounded text-[9px] font-mono border border-zinc-800 text-zinc-500">
                          step: {t.iteration}
                        </span>
                        {t.fired ? (
                          <span className="px-1.5 py-0.5 rounded text-[9px] bg-amber-400 text-black font-bold">FIRED</span>
                        ) : (
                      ...
                      <div className="text-[10px] text-zinc-500 flex items-center gap-2.5 flex-wrap">
                        <span>Premise Act: <strong>{(t.sourceActivation * 100).toFixed(1)}%</strong></span>
                        <span>Threshold Limit: <strong>{(t.threshold * 100).toFixed(0)}%</strong></span>
                        <span>Compiles: <strong className={t.fired ? 'text-emerald-400' : 'text-zinc-600'}>{t.fired ? 'SUCCESS' : 'FALSE'}</strong></span>
                      </div>
                      {!t.fired && t.skipReason && (
                        <div className="text-zinc-600 italic text-[10px] pt-1 border-t border-zinc-900/60 mt-1 font-sans">{t.skipReason}</div>
                      )}
                    </div>
                  </div>
                ))
      ...
      {/* 3. RIGHT COLUMN - Active Spread Activation Dashboard & Facts Table */}
      <div className="w-[400px] border-l border-zinc-800 flex flex-col min-w-0 bg-zinc-950/40">
        <div className="p-4 border-b border-zinc-800 bg-zinc-900/10 flex shrink-0 justify-between items-center">
          <div>
            <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
              <TrendingUp className="w-4 h-4 text-indigo-400" /> System State Metrics
            </h3>
            <p className="text-[9px] text-zinc-500 mt-0.5">Spacial activation vectors & logical derivations</p>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0">
          ...
          <div className="space-y-2.5">
            ...
            {topActivated.length === 0 ? (
              <div className="p-4 rounded-lg border border-zinc-800/80 bg-zinc-950/20 text-center text-zinc-600 font-mono text-[10px]">
                Vector space completely quiet. (0% active)
              </div>
            ) : (
          ...
          <div className="space-y-2.5">
            ...
            {facts.length === 0 ? (
              <div className="p-6 rounded-lg border border-dashed border-zinc-800 text-center text-zinc-600 font-mono text-[10px]">
                No derived facts. Try lowering rule thresholds.
              </div>
            ) : (
              <div className="space-y-3">
                {facts.map((f) => (
                  <div key={f.id} className={`p-3.5 rounded-xl border space-y-2.5 ${!f.fedBack ? 'border-amber-400/25 bg-amber-400/5' : 'border-zinc-800 bg-zinc-900/30'}`}>
                    <div className="text-xs text-zinc-200 font-mono break-words leading-relaxed">
                      {f.fact}
                    </div>
                    ...
                    {f.proofPath.length > 0 && (
                      <div className="space-y-2.5 pt-1">
                        <div className="text-[9px] text-zinc-500 uppercase tracking-wider font-bold font-mono flex items-center gap-1">
                          <GitFork className="w-3 h-3 text-violet-400" /> Multi-hop Derivation Path
                        </div>
                        <div className="space-y-2 pl-1 border-l border-zinc-800">
                          {f.proofPath.map((step, i) => (
                            <div key={i} className="text-[10px] text-zinc-400 pl-3 relative">
                              <div className="absolute left-0 top-1.5 w-1.5 h-1.5 rounded-full bg-violet-500 -translate-x-3.5"></div>
                              ...
                              <div className="text-[9px] text-zinc-500 font-mono mt-0.5 flex items-center gap-1.5">
                                <span>Activation: <strong>{(step.premiseActivation * 100).toFixed(0)}%</strong></span>
                                {step.typeResolution && (
                                  <>
                                    <span>·</span>
