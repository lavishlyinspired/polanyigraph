    ...
    <div className="relative w-full h-full">
      ...
      >
        ...
        <g transform={`translate(${pan.x}, ${pan.y}) scale(${zoom})`}>
          ...
          {edges.map((edge) => {
            ...
            // Define custom color palettes for each relationship type to make the graph incredibly premium
            const relationPalettes: Record<string, { stroke: string; fill: string; border: string; textClass: string }> = {
              hasDomicile: { stroke: '#818cf8', fill: '#1e1b4b', border: '#818cf8', textClass: 'fill-indigo-200' }, // Indigo
              issuedBy: { stroke: '#fb7185', fill: '#4c0519', border: '#fb7185', textClass: 'fill-rose-200' }, // Rose
              denominatedIn: { stroke: '#fbbf24', fill: '#451a03', border: '#fbbf24', textClass: 'fill-amber-200' }, // Amber
              regulates: { stroke: '#34d399', fill: '#064e3b', border: '#34d399', textClass: 'fill-emerald-200' }, // Emerald
              issuerDomicile: { stroke: '#2dd4bf', fill: '#115e59', border: '#2dd4bf', textClass: 'fill-teal-200' }, // Teal
            };

            const palette = relationPalettes[edge.type] || { stroke: '#a1a1aa', fill: '#18181b', border: '#52525b', textClass: 'fill-zinc-300' };

            const strokeColor = isProof ? '#0ea5e9' : isActive ? '#ffffff' : palette.stroke;
            const markerEnd = isProof ? 'url(#arrow-proof)' : isActive ? 'url(#arrow-active)' : 'url(#arrow)';
            
            const labelLength = edge.type.length;
            const pillWidth = Math.max(52, labelLength * 5.8 + 12);
            const pillHeight = 15;

            return (
              <g key={edge.id}>
                {/* Glow Underlay for active or proof edges */}
                {(isProof || isActive) && (
                  <line
                    x1={source.x} y1={source.y} x2={target.x} y2={target.y}
                    stroke={isProof ? '#0ea5e9' : '#ffffff'}
                ...
                <line
                  x1={source.x} y1={source.y} x2={target.x} y2={target.y}
                  stroke={strokeColor}
                  strokeWidth={isProof ? 1.75 : isActive ? 1.5 : 1.2}
                  opacity={isProof ? 1 : isActive ? 0.95 : 0.55}
                ...
                {/* Sleek pill/badge overlay for edge relation type */}
                <g transform={`translate(${midX}, ${midY})`}>
                  <rect
                    ...
                    height={pillHeight}
                    rx={6}
                    fill={isProof ? '#082f49' : isActive ? '#27272a' : palette.fill}
                    stroke={isProof ? '#38bdf8' : isActive ? '#ffffff' : palette.border}
                    strokeWidth={1.2}
                    strokeOpacity={isProof ? 0.9 : isActive ? 0.9 : 0.8}
                  ...
                  <text
                    ...
                    textAnchor="middle"
                    className={`text-[8.5px] font-mono font-bold tracking-widest uppercase pointer-events-none select-none ${
                      isProof ? 'fill-sky-300' : isActive ? 'fill-white' : palette.textClass
                    }`}
                  >
                    {edge.type}
                  </text>
          ...
          {nodes.map((node) => {
            ...
            return (
              ...
              >
                ...
                {isProofNode && (
                  <circle r={r + 11} fill="none" stroke="#22d3ee" strokeWidth={1.5} strokeDasharray="4 3" opacity={0.7} className="pointer-events-none" />
                )}

                {/* UI_PLAN.md §9.3.4: implicit-fact marker, distinct from `derived`
                    (amber) and proof-path (sky dashed) -- sky dot badge. */}
                {hasImplicitFact && (
                  <g transform={`translate(${r - 4}, ${-r + 4})`} className="pointer-events-none">
                    <circle r={6} fill="#22d3ee" stroke="#18181b" strokeWidth={1.5} />
      ...
      <div className="absolute top-3 right-3 flex flex-col gap-1">
        ...
        <button
          ...
          className={`w-7 h-7 rounded flex items-center justify-center transition-colors border ${
            showProofPath ? 'bg-sky-400/20 border-sky-400/40 text-sky-400' : 'bg-zinc-900 border-zinc-800 text-zinc-500 hover:text-white'
          }`}
