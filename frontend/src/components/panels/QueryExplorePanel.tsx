// Sidebar "Query" tab (UI_REFACTOR_PLAN.md §2): pill switcher over the
// existing, unchanged QueryPanel/TripleStorePanel/OntologyPanel. Ported
// from .claude/docs/mocks/polanyigraph.
  ...
  return (
    <div className="h-full flex flex-col bg-zinc-950 text-zinc-100 overflow-hidden">
        ...
        <div className="w-full grid grid-cols-3 gap-1 p-0.5 rounded-lg bg-zinc-950 border border-zinc-800/80">
          <button
            ...
            className={`py-1 text-[10px] font-semibold rounded flex items-center justify-center gap-1.5 transition-all duration-150 ${
              mode === 'pattern' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
          ...
          <button
            ...
            className={`py-1 text-[10px] font-semibold rounded flex items-center justify-center gap-1.5 transition-all duration-150 ${
              mode === 'triples' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
          ...
          <button
            ...
            className={`py-1 text-[10px] font-semibold rounded flex items-center justify-center gap-1.5 transition-all duration-150 ${
              mode === 'ontology' ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
      ...
      <div className="flex-1 overflow-hidden">
        {mode === 'pattern' ? <QueryPanel /> : mode === 'triples' ? <TripleStorePanel /> : <OntologyPanel />}
      </div>
    </div>
  );
}
