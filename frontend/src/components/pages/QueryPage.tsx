// Query Lab (UI_REFACTOR_PLAN.md §3.4): canvas-free duplicate of the
// sidebar's Query pill + Triples pill -- same real query/path-finding store
// actions and GET /query/{graphId}/triples, with a real filterable <table>
// instead of the sidebar's stacked cards. Ported from
// .claude/docs/mocks/polanyigraph, fixed invalid Tailwind utilities
// (zinc-850 isn't a real shade, h-8.5/bg-zinc-750 aren't real classes) and
// dropped an unused duplicate `HelpIcon` import.
import { useEffect, useState } from 'react';
import { Search, AlertCircle, GitFork, Database, ListFilter, HelpCircle, ArrowRight, ShieldCheck, Sparkles } from 'lucide-react';
...
export function QueryPage() {
  ...
  const [bfsSource, setBfsSource] = useState('');
  ...
  return (
    <div className="h-full flex-1 flex overflow-hidden bg-zinc-950">
      {/* Datalog Pattern Console + BFS Finder */}
      <div className="w-[420px] border-r border-zinc-800 flex flex-col min-w-0 bg-zinc-950/20">
        <div className="p-4 border-b border-zinc-800 bg-zinc-900/10 shrink-0">
          <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
            <Search className="w-4 h-4 text-blue-400" /> Datalog Pattern Console
          </h3>
        ...
        <div className="p-4 border-b border-zinc-800 shrink-0 space-y-3">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-zinc-900 border border-zinc-800 rounded-lg text-xs text-white placeholder:text-zinc-700 font-mono px-3 h-9 focus:outline-none focus:border-blue-500 transition-colors"
            />
            <button onClick={() => void handleRunQuery(queryText)} className="h-9 px-4 rounded-lg bg-blue-600 text-onaccent hover:bg-blue-500 font-bold text-xs flex items-center justify-center gap-1.5 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-4 border-b border-zinc-800 min-h-0">
          <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider font-mono mb-2.5">Console Pattern Matches</div>
          ...
          {queryError ? (
              ...
              {queryResults.map((r, i) => (
                <div key={i} className={`p-3 rounded-lg border ${r.derived ? 'border-amber-400/20 bg-amber-400/5' : 'border-zinc-800 bg-zinc-900/40'}`}>
                  ...
                  <div className="flex items-center gap-2 mt-1.5 pt-1.5 border-t border-zinc-800/40">
                    {r.derived ? (
                      <span className="px-1.5 py-0.5 rounded text-[8px] bg-amber-400 text-badgeink font-bold font-mono">DERIVED</span>
                    ) : (
            ...
            <div className="text-center py-12 text-zinc-600">
              <HelpCircle className="w-8 h-8 mx-auto mb-2 text-zinc-800" />
              ...
              <div className="mt-3.5 space-y-1.5 text-left max-w-xs mx-auto">
                {QUERY_EXAMPLES.map((ex, i) => (
                  <button
                    ...
                    className="block w-full text-left p-2 rounded border border-zinc-800 hover:border-zinc-700 text-zinc-500 hover:text-zinc-300 transition-colors font-mono text-[10px] truncate"
                  >
                    {ex}
                  </button>
                ))}
        ...
        <div className="p-4 bg-zinc-900/10 space-y-3 shrink-0">
          <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider font-mono flex items-center gap-1.5">
            <GitFork className="w-3.5 h-3.5 text-blue-400" /> Explainable BFS Path Router
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <input
              ...
              className="bg-zinc-900 border border-zinc-800 rounded-lg text-xs text-white placeholder:text-zinc-700 px-2.5 h-[34px] focus:outline-none focus:border-blue-500"
            />
            <input
              value={bfsTarget}
          ...
          <button
            ...
            className="w-full h-[34px] bg-blue-600 hover:bg-blue-500 text-onaccent text-xs font-bold rounded-lg flex items-center justify-center gap-1.5 transition-all disabled:opacity-30"
          >
                  ...
                  <div className="flex flex-wrap items-center gap-1">
                    {bfsPathResult.path.map((label, i) => (
                      <span key={i} className="flex items-center gap-1">
                        ...
                        <span className="px-2 py-0.5 rounded bg-zinc-900 text-[10px] text-zinc-300 font-mono border border-zinc-800">{label}</span>
                      </span>
                    ))}
                  </div>
                  <div className="p-2 rounded bg-sky-400/5 border border-sky-400/10 text-[10px] leading-relaxed text-zinc-300 font-sans">
                    <strong className="text-sky-400 font-mono text-[9px] block uppercase mb-0.5">Proof explanation</strong>
                    {bfsPathResult.proof}
                  </div>
                </div>
              ) : (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="p-4 border-b border-zinc-800 bg-zinc-900/10 shrink-0 flex items-center justify-between">
          <div>
            <h2 className="text-[13px] font-bold text-white tracking-wider uppercase flex items-center gap-2">
              <Database className="w-4 h-4 text-blue-400" /> Relational Triple Ledger
            </h2>
          ...
          <span className="px-2.5 py-1 rounded text-[10px] bg-blue-500/10 border border-blue-500/20 text-blue-400 font-mono font-bold">{totalTriples} total facts</span>
        </div>
        ...
        <div className="p-4 border-b border-zinc-800 bg-zinc-950/20 shrink-0 flex flex-wrap gap-3 items-center">
          ...
          <input
            ...
            placeholder="Subject..."
            className="bg-zinc-900 border border-zinc-800 rounded-lg text-[11px] text-white px-2.5 h-8 placeholder:text-zinc-700 w-36 focus:outline-none focus:border-blue-500"
          />
          <input
            value={predFilter}
            ...
            {(['all', 'base', 'derived'] as const).map((type) => (
              <button
                ...
                className={`px-3 py-1 text-[10px] font-mono uppercase font-bold rounded-md transition-all duration-150 ${
                  typeFilter === type ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'
                }`}
              >
                {type}
              </button>
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {loadingTriples ? (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-[11px] font-mono text-zinc-300 divide-y divide-zinc-800">
                {filteredTriples.map((t, i) => (
                  <tr key={i} className="hover:bg-zinc-900/35 transition-colors group">
                    ...
                    <td className="p-3 text-sky-400 font-bold max-w-xs truncate">{t.predicate}</td>
            ...
            <div className="h-64 flex flex-col items-center justify-center text-center text-zinc-600 font-mono py-10">
              <Database className="w-10 h-10 text-zinc-800 mb-3" />
